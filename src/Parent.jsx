...
const handlerequest = () => {
  ...
  Axios.post("http://localhost:3001/api/getstudent", {regid: regid})
  .then((response) => {

if(response.data.length>0){  
    if(approval=="Yes"){
      alert("Approved!!");
      Axios.post("http://localhost:3001/api/parentapprove", {regid: regid})
      }
      else{
        alert("Disaaproved!!");
  ...
  return (<>
      <AppBar position="static" color="inherit" elevation={0} className={classes.appBar}>
        <Toolbar className={classes.toolbar}>
          ...
          <Button href="/Login2" color="inherit" variant="filled" className={classes.link}>
            Logout
